...
| `check.js` | `npm run check` | Lints all asset frontmatter (name ↔ path, the `consumers` enum, the `cc-plugin` ⟺ `skills/` and uppercase-`SKILL.md` invariants, declared placeholders, schema) and fails if either generated file is stale — offline, no API key |
